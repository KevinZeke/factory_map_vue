...
//            }
     ...
     * @return SqlResult
     */
    function prepare_execute_res($stmt)
    {
        $stmt->execute() or die('sql语句出错 : ' . $this->mysqli->error);
        $result = $stmt->get_result() or die('结果集获取出错 : ' . $this->mysqli->error);
     ...
     * @return bool
     */
    function prepare_execute($stmt)
    {
        return $stmt->execute() or die('sql语句出错 : ' . $this->mysqli->error);
    }
